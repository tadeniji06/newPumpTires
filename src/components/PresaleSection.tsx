	...
	const validateAmount = (value: string): boolean => {
		...
		if (paymentType === "usdc") {
			...
			const balance = usdcBalance
				? parseFloat(formatUnits(usdcBalance.value, 6))
				: 0;
			...
			if (numAmount < MIN_PLS) {
				setError(
					`Minimum purchase is ${MIN_PLS.toLocaleString()} PLS`
				);
				return false;
			}

			const balance = plsBalance
				? parseFloat(formatUnits(plsBalance.value, 18))
				: 0;
			if (numAmount > balance) {
			...
			<div className='space-y-4'>
						...
						>
							...
							{plsBalance && (
								<div className='text-xs text-gray-500 mt-1'>
									Balance:{" "}
									{parseFloat(
										formatUnits(plsBalance.value, 18)
									).toFixed(0)}
								</div>
							)}
						</button>
				...
				{success && (
					<div className='flex items-center gap-2 p-3 bg-primary-green/10 border border-primary-green/30 rounded-lg text-primary-green'>
						...
						<p className='text-sm'>
							Transaction successful! Thank you for participating.
						</p>
					</div>
				)}

				{/* Buy Button or Connect Wallet Button */}
				{!isConnected ? (
					<div className='w-full'>
						<WalletConnectButton title='Connect Wallet to Participate' />
					</div>
				) : (
					<button
				...
				{(plsHash || usdcHash) && (
					<div className='text-sm text-gray-400 text-center'>
						<a
							href={`https://scan.pulsechain.com/tx/${
								plsHash || usdcHash
							}`}
							target='_blank'
				...
				<div className='text-xs text-gray-500 space-y-1 pt-2'>
					...
					<p>
						• Receiving address: {recieveWallet.slice(0, 6)}...
						{recieveWallet.slice(-4)}
					</p>
				</div>
			</div>
		</section>
